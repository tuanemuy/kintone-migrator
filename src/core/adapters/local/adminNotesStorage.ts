...
export class LocalFileAdminNotesStorage implements AdminNotesStorage {
  ...
  async get(): Promise<{ content: string; exists: boolean }> {
    try {
      ...
      throw new SystemError(
        ...
        `Failed to read admin notes file: ${this.filePath}`,
        error,
      );
    }
  }
  ...
  async update(content: string): Promise<void> {
    try {
      ...
      throw new SystemError(
        SystemErrorCode.StorageError,
        `Failed to write admin notes file: ${this.filePath}`,
        error,
      );
    }
  }
