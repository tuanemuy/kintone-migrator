...
describe("SeedParser", () => {
  ...
  it("ユーザーエンティティ配列をパースする", () => {
    const input = {
      key: "code",
      records: [
        { code: "001", assignee: [{ code: "user1" }, { code: "user2" }] },
      ],
    };
    const result = SeedParser.parse(input);
  ...
  it("サブテーブル行をパースする", () => {
    const input = {
      ...
      records: [
        {
          code: "001",
          items: [{ product: "商品A", quantity: 1, price: 1000 }],
        },
      ],
    };
    const result = SeedParser.parse(input);
      ...
      records: [
        {
          code: "001",
          items: [
            { product: "商品A", options: ["オプション1", "オプション2"] },
          ],
        },
      ],
    };
  ...
  it("keyなし時にキーフィールド検証がスキップされる", () => {
    const input = {
      records: [{ name: "テスト1", code: "001" }, { name: "テスト2" }],
    };
    const result = SeedParser.parse(input);
    expect(result.key).toBeNull();
    expect(result.records).toHaveLength(2);
  ...
  it("recordsが配列でない場合にエラーをスローする", () => {
    try {
      SeedParser.parse({ key: "code", records: "not_array" });
